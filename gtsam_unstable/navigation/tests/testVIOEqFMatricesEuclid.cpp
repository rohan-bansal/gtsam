...
VIOGroup Group0() { return VIOGroup::Identity(0); }
VIOGroup Group1() {
  const SOT3 q1(SO3::Expmap((Vector3() << 0.02, -0.01, 0.03).finished()), 1.1);
  return VIOGroup(
...
VIOGroup Group3() {
  const SOT3 q1(SO3::Expmap((Vector3() << 0.02, -0.01, 0.03).finished()), 1.1);
  const SOT3 q2(SO3::Expmap((Vector3() << -0.01, 0.03, -0.02).finished()), 0.95);
  const SOT3 q3(SO3::Expmap((Vector3() << 0.01, 0.02, 0.01).finished()), 1.05);
  return VIOGroup(
      MakeA(Rot3::RzRyRx(0.03, -0.02, 0.01), Point3(0.05, -0.01, 0.02),
            Vector3(0.01, -0.02, 0.03)),
      (Vector6() << 0.01, 0.0, -0.01, 0.02, -0.01, 0.03).finished(),
...
TEST(VIOEqFMatrices, SimpleRegressionN0) {
  ...
  const VIOGroup X = VIOGroup::Identity(0);
